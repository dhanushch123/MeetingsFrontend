        ...
        <div className={styles.top}> 
         ...
         {(activeIndex == 1 || activeIndex == 2)? (<button style={{backgroundColor : '#00C35F'}} className={styles.AR}><img onClick={async()=>{
            ...
            try{
                ...
                let request = await axios.put(`http://localhost:2500/user/setStatus/${item._id}`,{status : "accepted"},{withCredentials : true})
         ...
         {(activeIndex == 0 || activeIndex == 1)?(<button style={{backgroundColor : "#ED0000"}} className={styles.AR}><img onClick={async()=>{
            ...
            try{
                ...
                let request = await axios.put(`http://localhost:2500/user/setStatus/${item._id}`,{status : "rejected"},{withCredentials : true})
                setShowParticipants(false)
            }
            catch(err)
            {
